...
import { ProfileTable } from './profilesTab/ProfileTable';
import { ProfileControls } from './profilesTab/ProfileControls';
import { ProfilePagination } from './profilesTab/ProfilePagination';
import { ScriptModal, DeleteConfirmDialog, ImagePreviewModal } from './modals/ProfileModals';
import { getLastExecutionTime } from './profilesTab/profileUtils';
import { useProfileMutations } from './profilesTab/useProfileMutations';
...
export default function ProfilesPanel({
  ...
  const [scheduledProfilesCount, setScheduledProfilesCount] = useState(0);
  ...
  useEffect(() => {
    ...
    const fetchScheduledProfiles = async () => {
      try {
        const response = await fetch('/api/scheduledProfiles');
        const data = await response.json();
        setScheduledProfilesCount(data.length || 0);
      } catch (error) {
        console.error('Failed to fetch scheduled profiles:', error);
        setScheduledProfilesCount(0);
      }
    };

    fetchSettings();
    fetchScheduledProfiles();
  }, []);
  ...
  const handleLogClick = async (profile: Profile) => {
    try {
      const response = await fetch(`/api/profiles/${profile.id}/log`);
      if (response.ok) {
        const data = await response.json();
  ...
  const showScriptDetails = async (profileId: number) => {
    try {
      ...
      if (response.ok) {
        ...
        const createResponse = await fetch(`/api/profiles/${profileId}/script`, {
          method: "POST",
  ...
  const refreshScheduledProfiles = async () => {
    const response = await fetch('/api/scheduledProfiles');
    const data = await response.json();
    setScheduledProfilesCount(data.length || 0);
    return data;
  ...
  return (
    <div className="p-6">
      <ProfileControls
        ...
        scheduledProfilesCount={scheduledProfilesCount}
        totalProfiles={profiles.length}
        onRefreshScheduledProfiles={refreshScheduledProfiles}
      />
